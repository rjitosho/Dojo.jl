"""
    FlyingVine <: Environment
"""
struct FlyingVine end
...
function flyingvine(; 
    ...
    mechanism = get_mechanism(:flyingvine, 
        timestep=timestep, 
        gravity=gravity, 
        mass=mass, 
        vine_len=vine_len, 
    ...
    initialize!(mechanism, :flyingvine)
    ...
    env = Environment{FlyingVine, TYPES...}(mechanism, representation, aspace, ospace,
        x, fx, fu,
...
function visualize(env::Environment{FlyingVine}, traj::Vector{Vector{T}}; 
    axis=true, 
    grid=true) where T
