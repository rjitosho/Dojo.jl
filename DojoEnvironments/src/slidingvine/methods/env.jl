"""
    SlidingVine <: Environment
"""
struct SlidingVine end

function slidingvine(; 
    ...
    mechanism = get_mechanism(:slidingvine, 
        timestep=timestep, 
        gravity=gravity, 
        mass=mass, 
        vine_len=vine_len, 
    ...
    initialize!(mechanism, :slidingvine)
    ...
    env = Environment{SlidingVine, TYPES...}(mechanism, representation, aspace, ospace,
        x, fx, fu,
...
function visualize(env::Environment{SlidingVine}, traj::Vector{Vector{T}}; 
    axis=true, 
    grid=true) where T
